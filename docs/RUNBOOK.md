...
- **Fix (recommended)**: Deploy the frontend and backend under the **same registrable domain**. Two ways to get there:

  **Option 1 -- reverse-proxy rewrite (fully same-*origin*, no code changes needed):** the SPA's own domain proxies `/api/*` (and `/uploads/*`) through to the backend service, so the browser never sees a different host at all.
  - **Render** (Static Site + separate Web Service): Dashboard -> Static Site -> Redirects/Rewrites -> add two **Rewrite** rules (not Redirect): `/api/*` -> `https://<your-backend>.onrender.com/api/*` and `/uploads/*` -> `https://<your-backend>.onrender.com/uploads/*`. Use the backend's **public** `onrender.com` URL, not its internal hostname (a known cause of 502s). Set the frontend's `VITE_API_URL=/api` (relative) and redeploy.
  - **Vercel**: add a `rewrites` entry routing `/api/*` (and `/uploads/*`) to the backend's URL, then set `VITE_API_URL=/api` at build time.
  - **Netlify**: an equivalent proxy redirect in `netlify.toml` (`from = "/api/*"`, `to = "https://<backend>/api/:splat"`, `status = 200`).
  - **Cloudflare Pages/Workers**: a Worker route or Pages Function proxying `/api/*` to the backend origin.

  **Option 2 -- subdomains of the same domain (same-*site*, not same-origin, but enough to avoid third-party-cookie treatment):** e.g. `app.example.com` for the frontend, `api.example.com` for the backend.
  - **Render**: add each subdomain as a Custom Domain on its respective service (Dashboard -> service -> Custom Domains), create the CNAME record your domain registrar/DNS provider asks for pointing at that service's `onrender.com` hostname, then click Verify. Render auto-provisions and renews the TLS certificate for each.
  - Set `VITE_API_URL=https://api.example.com/api` (absolute, since it's now genuinely a different hostname) on the frontend.
  - Set the new `COOKIE_DOMAIN=.example.com` on the backend (see `docs/ENV.md`) -- this is what makes the cookie shared across both subdomains instead of host-only to the API subdomain alone. Without it, subdomains alone do *not* fix the logout issue.
  - Keep `FRONTEND_URL=https://app.example.com` accurate for CORS -- unlike Option 1, the browser *does* see this as a cross-origin request (different subdomain), so CORS still applies even though it's same-site.

  Either option turns the refresh-token cookie into a first-party (or at least same-site) cookie, sidestepping third-party-cookie restrictions.
- **Fix (if neither is possible right now)**: confirm `sameSite: 'none'` + `secure: true` are actually in effect (`NODE_ENV=production` on the backend) and that all domains are served over HTTPS; test specifically in an *installed* PWA, not just a regular tab, since that's where third-party-cookie enforcement is strictest. Expect this to remain unreliable on iOS Safari PWAs regardless -- Option 1 or 2 above is the only fully reliable fix.
